...
function loadData() {
  ...
  const FAC_MATCHERS = {
    ...
    "Central Service Unit": [
      ...
      "marketing",
      "careers",
      "learning and research technology team",
      "directorate",
  ...
  function detect(entry = "", TEXT_MATCHERS) {
    function matches(matchers) {
      return _.some(matchers, matcher =>
        entry
          .toLowerCase()
          .replace(/\s+/g, "")
          .includes(matcher.replace(/\s+/g, ""))
      );
    }
  ...
  function extractRelevantData(result) {
    ...
    return {
      extractedData: rowsWithNames.map(row => ({
        id: row["name"],
        name: row["name"],
  ...
  function formatForCytoscape(sData, pData) {
    let personData = sData.extractedData;
    return {
      nodes: _.flattenDeep([
        personData.map(row => ({ data: row })), //store extracted data
        pData.map(row => ({ data: row })),
        _.keys(FAC_MATCHERS).map(name => ({
          data: { id: name, name: name, programme: "", type: "school" }
        })), //school nodes
      ...
      edges: _.flattenDeep([
        personData.map(row => ({
          data: { source: row.name, target: row.fac, type: "school" }
        })), //edges(conection lines) for school to person
        // personData.map(row => {
        ...
        pData.map(row => {
          return row.recommendations.map(name => ({
            data: { source: row.name, target: name, type: "project" }
          })); //edges between projects and people
        })
      ])
    };
  }
