...
function parseStyles(allNodes, colorList, styleList, data) {
  ...
  Object.keys(cssColors).forEach(value => {
    if (cssColors[value].constructor !== Array) {
      $(":root")
        .get(0)
        .style.setProperty("--" + value.replaceAll(".", "-"), cssColors[value]);
    ...
    style.subtype.forEach(subName => {
      ...
      data = data.insertBefore(
        ...
        styleString.replaceSelector(
          ...
          subName,
          nodeColor,
          typeAr.caseIndexOf(typeStyle.subtype[typeStyle.subtype.length - 1]) +
            1
        )
      );
    });
...
function sortBySubType(a, b, styleList) {
  ...
  if (!atype || !btype) {
    ...
    if (!aStyle || !bStyle || atype !== btype) {
      var orderA = atype.color.isHexColor()
        ? typeAr.caseIndexOf(atype.label) + 10
        : atype.color;
      var orderB = atype.color.isHexColor()
        ? typeAr.caseIndexOf(btype.label) + 10
        : btype.color;
    } else {
      var orderA = aStyle.color.isHexColor()
        ? subAr.caseIndexOf(aStyle.label) + 10
        : aStyle.color;
      var orderB = bStyle.color.isHexColor()
...
function navPadding() {
  ...
  var headerHeight =
    $("#header").height() + parseInt($("#header").css("top"), 10);
  var controlWidth =
    $("#controls").width() + parseInt($("#controls").css("left"), 10);
  var toggleWidth =
    $("#toggle").width() + parseInt($("#toggle").css("right"), 10);
